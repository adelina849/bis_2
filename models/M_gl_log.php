	...
	class M_gl_log extends CI_Model 
	{
		...
		function notif_count_pasien_dokter($kode_kantor,$id_dokter,$jabatan)
		{
			...
			*/
			...
			$query = 
				"
					SELECT COUNT(id_pengajuan) AS JUMLAH FROM tb_pengajuan WHERE kode_kantor = '".$kode_kantor."' AND DATE(tgl_ins) = DATE(NOW()) AND kode_program = '';
				";
			...
			*/
			...
			$query =
				"
					...
					LEFT JOIN tb_costumer AS B ON A.id_costumer_pangaju = B.id_costumer AND A.kode_kantor = B.kode_kantor
					WHERE DATE(A.tgl_ins) = DATE(NOW()) AND kode_program = '' ORDER BY A.tgl_ins ASC;
				";
