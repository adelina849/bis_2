<?php
	class M_gl_dashboard extends CI_Model 
	{
		...
		function jum_karyawan($kode_kantor)
		{
			$query = "SELECT COUNT(id_karyawan) AS JUM_KARYAWAN FROM tb_karyawan WHERE kode_kantor = '".$kode_kantor."';";
		...
		function jum_departemen($kode_kantor)
		{
			$query = "SELECT COUNT(id_dept) AS JUM_DEPT FROM tb_dept WHERE kode_kantor = '".$kode_kantor."';";
		...
		function jum_jabatan($kode_kantor)
		{
			$query = "SELECT COUNT(id_jabatan) AS JUM_JABATAN FROM tb_jabatan WHERE kode_kantor = '".$kode_kantor."';";
		...
		function statistik_surat_masuk($kode_kantor)
		{
			$query = "
						...
						LEFT JOIN
						(
							SELECT DATE(tgl_masuk) AS DT,COUNT(id_surat) AS JUMLAH_KUNJUNGAN
							FROM tb_surat 
							WHERE kode_kantor = '".$kode_kantor."' AND in_out = 'IN'
		...
		function statistik_pengajuan_proposal($kode_kantor)
		{
			$query = "
						SELECT RIGHT(A.DATE_LIST,2) AS DATE_LIST,COALESCE(B.JUMLAH_KUNJUNGAN,0) AS JUM_KUNJUNGAN
						FROM
						...
						LEFT JOIN
						(
							SELECT DATE(tgl_terima) AS DT,COUNT(id_pengajuan) AS JUMLAH_KUNJUNGAN
							FROM tb_pengajuan 
