<?php
	class M_gl_costumer extends CI_Model 
	{
		...
		function list_costumer_biasa($cari,$limit,$offset)
		{
			...
			"
				SELECT
					...
					, COALESCE(E.name,'') AS PROVINSI2
					, COALESCE(F.name,'') AS KABKOT2
					, COALESCE(G.name,'') AS KECAMATAN2
					, COALESCE(H.name,'') AS DESA2
				...
				LEFT JOIN provinces AS E ON A.provinsi = E.id
				LEFT JOIN tb_kabkot AS F ON A.kabkota = F.id
				LEFT JOIN tb_kecamatan AS G ON A.kecamatan = G.id
				LEFT JOIN tb_desa AS H ON A.desa = H.id
		...
		function list_costumer_biasa_history_ajuan_mustahik($cari,$limit,$offset)
		{
			$query =
			"
				SELECT
					...
					,CASE 
							...
							END
						AS USIA
					,COALESCE(I.CNT,0) AS AJUAN
					,COALESCE(I.TGL,0) AS TGL_AJUAN
					,COALESCE(J.CNT,0) AS ASHNAF
					,COALESCE(J.TGL,0) AS TGL_ASHNAF
				FROM tb_costumer AS A
				LEFT JOIN tb_kat_costumer AS B ON A.id_kat_costumer = B.id_kat_costumer AND A.kode_kantor = B.kode_kantor
				LEFT JOIN
				(
				...
				LEFT JOIN
				(
					SELECT kode_kantor,id_costumer_pangaju AS id_costumer, COUNT(id_pengajuan) AS CNT, MAX(tgl_terima) AS TGL 
					FROM tb_pengajuan AS A
					GROUP BY kode_kantor,id_costumer_pangaju
				) AS I ON A.id_costumer = I.id_costumer AND A.kode_kantor = I.kode_kantor
				LEFT JOIN
				(
					SELECT kode_kantor,id_costumer_ashnaf AS id_costumer, COUNT(id_pengajuan) AS CNT, MAX(tgl_terima) AS TGL 
					FROM tb_pengajuan AS A
					GROUP BY kode_kantor,id_costumer_ashnaf
				) AS J ON A.id_costumer = J.id_costumer AND A.kode_kantor = J.kode_kantor
