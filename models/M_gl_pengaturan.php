	...
	function get_notifikasi_transaksi($kode_kantor)
	{
		...
		*/
		...
		$query = "
					...
					(
						SELECT 
							CASE WHEN (kode_program = '') THEN 1 ELSE 0 END AS PENDING
							,CASE WHEN ((kode_program = '') AND (nominal = 0) )  THEN 1 ELSE 0 END AS PEMERIKSAAN
							,CASE WHEN ((nominal > 0) AND (tgl_penyerahan = '0000-00-00')) THEN 1 ELSE 0 END AS PEMBAYARAN
						FROM tb_pengajuan 
						WHERE kode_kantor = '".$kode_kantor."' 
						AND DATE(tgl_terima) = DATE(NOW())
						-- AND kode_program = ''
					) AS AA
					;
		";
