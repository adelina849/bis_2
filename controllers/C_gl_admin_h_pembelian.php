	...
	function list_h_pembelian_penerimaan()
	{
		...
		else
		{
			...
			if(!empty($cek_ses_login))
			{
				...
				$list_h_pembelian_penerimaan = $this->M_gl_h_pembelian->list_h_pembelian_penerimaan($this->session->userdata('ses_kode_kantor'),$cari,$dari,$sampai,$order_by,$config['per_page'],$this->uri->segment(2,0));
				...
				$data = array('page_content'=>'gl_admin_h_pembelian_penerimaan','halaman'=>$halaman,'list_h_pembelian_penerimaan'=>$list_h_pembelian_penerimaan,'msgbox_title' => $msgbox_title,'sum_pesan' => $sum_pesan);
				$this->load->view('admin/container',$data);
			}
			else
			{
