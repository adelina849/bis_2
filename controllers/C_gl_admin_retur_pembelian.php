...
class C_gl_admin_retur_pembelian extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model(array('M_gl_supplier','M_gl_h_pembelian','M_gl_h_retur','M_gl_produk','M_gl_satuan_konversi'));
...
<tr>';
				...
				foreach($list_result as $row)
				{
					...
					echo'<td>'.$no.'</td>';
						...
						echo'<td><b>SUPPLIER : </b>'.$row->NAMA_SUPPLIER.'
						...
						echo'<td>
								...
								<a href="'.base_url().'gl-admin-retur-supplier/'.(md5($row->id_h_penjualan)).'" class="confirm-btn btn btn-success btn-block btn-flat" title="Apakah yakin akan mengubah retur '.$row->no_faktur.' ?" alt="Apakah yakin akan mengubah retur '.$row->no_faktur.' ?">UBAH</a>
								
								<a href="'.base_url().'gl-admin-mutasi-out-hapus-all?hapus='.(md5($row->id_h_penjualan)).'" class="confirm-btn btn btn-danger btn-block btn-flat" title="Apakah yakin akan menghapus retur '.$row->no_faktur.' ?" alt="Apakah yakin akan menghapus retur '.$row->no_faktur.' ?" >HAPUS</a>
							</td>';
						/*DENHGAN ENRYPTION */
