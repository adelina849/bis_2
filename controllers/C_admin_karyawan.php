...
class C_admin_karyawan extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model(array('M_karyawan','M_jabatan','M_departemen'));
	}
	...
	public function index()
	{
		...
		else
		{
			...
			if(!empty($cek_ses_login))
			{
				...
				$list_departmen = $this->M_departemen->list_departemen_limit('',100,0);
				
				$data = array('page_content'=>'king_admin_karyawan','halaman'=>$halaman,'list_karyawan'=>$list_karyawan,'list_jabatan'=>$list_jabatan,'no_karyawan'=>$no_karyawan,'list_departmen' => $list_departmen);
	...
	public function hapus()
	{
		...
		if(!empty($hasil_cek))
		{
			...
			$this->M_log->simpan(
									...
									"Melakukan penghapusan karyawan ".$hasil_cek->nama_karyawan." dengan id : ".$id." ",
									$this->session->userdata('ses_kode_kantor')
								);
		}
		header('Location: '.base_url().'admin-karyawan');
