		...
		else
		{
			...
			if(!empty($cek_ses_login))
			{
				...
				if(!empty($data_costumer))
				{
					...
					$data = array('page_content'=>'gl_pusat_costumer_detail','halaman'=>$halaman,'list_rekam_medis'=>$list_rekam_medis,'msgbox_title' => $msgbox_title,'sum_pesan' => $sum_pesan,'data_costumer' => $data_costumer,'kode_kantor' => $kode_kantor);
					$this->load->view('pusat/container',$data);
				}
				else
				{
