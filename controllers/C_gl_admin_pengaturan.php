...
class C_gl_admin_pengaturan extends CI_Controller {
	...
	function notifikasi_pasien()
	{
		...
		echo'<ul class="dropdown-menu">';
		  echo'<li style="color:red;background-color:purple;" class="header">'.$this->session->userdata('ses_nama_karyawan').' ada '.$jumlah_pasien.' pengajuan yang menunggu</li>';
		  echo'<li>';
						...
						if(!empty($notif_list_pasien_tunggu))
						{
							...
							foreach($list_result as $row)
							{
								...
								echo'<li><!-- start message -->';
									//echo'<a href="'.base_url().'gl-admin-disposisi/'.md5($row->id_pengajuan).'">';
									echo'<a href="'.base_url().'gl-admin-disposisi/">';
									...
									echo'</h4>';
									echo'<p>'.word_limiter($row->perihal,10).'</p>';
									echo'</a>';
								echo'</li>';
