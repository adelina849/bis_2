...
class C_gl_admin_kode_akun extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model(array('M_gl_kode_akun'));
			...
			if(!empty($cek_ses_login))
			{
				if((!empty($_GET['cari'])) && ($_GET['cari']!= "")  )
				{
					$cari = "WHERE kode_kantor = '".$this->session->userdata('ses_kode_kantor')."' 
							AND (kode_akun LIKE '%".str_replace("'","",$_GET['cari'])."%' OR nama_kode_akun LIKE '%".str_replace("'","",$_GET['cari'])."%')";
				}
				else
				{
					$cari = "WHERE kode_kantor = '".$this->session->userdata('ses_kode_kantor')."'";
				...
				$cari_kode_akun_induk = "WHERE kode_kantor = '".$this->session->userdata('ses_kode_kantor')."' AND kode_akun_induk = '' ";
				$list_kode_akun_induk = $this->M_gl_kode_akun->list_kode_akun_limit($cari_kode_akun_induk,1000,0);
				
				$msgbox_title = " Kode Akuntansi";
				
				$data = array('page_content'=>'gl_admin_kode_akun','halaman'=>$halaman,'list_kode_akun'=>$list_kode_akun,'msgbox_title' => $msgbox_title,'list_kode_akun_induk'=>$list_kode_akun_induk);
