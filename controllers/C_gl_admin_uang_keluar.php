...
class C_gl_admin_uang_keluar extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model(array('M_gl_uang_keluar','M_gl_bank','M_gl_kode_akun'));
	...
	public function index()
	{
		...
		else
		{
			...
			if(!empty($cek_ses_login))
			{
				...
				$list_uang_keluar = $this->M_gl_uang_keluar->list_uang_keluar_limit($cari,$order_by,$config['per_page'],$this->uri->segment(2,0));
				...
				$msgbox_title = " Pengeluaran/Uang Keluar Pembiayaan";
				
				$data = array('page_content'=>'gl_admin_uang_keluar','halaman'=>$halaman,'list_uang_keluar'=>$list_uang_keluar,'msgbox_title' => $msgbox_title,'sum_pesan' => $sum_pesan,'list_bank'=>$list_bank,'get_no_uang_keluar'=>$get_no_uang_keluar,'sum_uang_keluar'=>$sum_uang_keluar);
		...
		else
		{
			...
			if(!empty($cek_ses_login))
			{
				...
				if(!empty($get_data_uang_keluar))
				{
					...
					$msgbox_title = " Tambah Pengeluaran/Uang Keluar Pembiayaan";
