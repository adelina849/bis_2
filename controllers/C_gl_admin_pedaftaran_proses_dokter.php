...
class C_gl_admin_pedaftaran_proses_dokter extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model(array('M_gl_h_penjualan','M_gl_costumer','M_gl_produk','M_gl_h_diskon','M_gl_d_diskon','M_gl_images','M_gl_bank','M_gl_h_penjualan_catat_untuk_fee','M_gl_media_transaksi','M_gl_hpp_jasa'));
	...
	{	
		...
		if((!empty($_POST['cari'])) && ($_POST['cari']!= "")  )
		{
			$cari = "WHERE A.kode_kantor = '".$this->session->userdata('ses_kode_kantor')."' 
					...
					AND ( TRIM(A.kode_produk) LIKE '%".str_replace("'","",$_POST['cari'])."%' OR REPLACE(A.nama_produk,'  ',' ') LIKE '%".str_replace("'","",$_POST['cari'])."%')";
		}
		else
		{
			$cari = "WHERE A.kode_kantor = '".$this->session->userdata('ses_kode_kantor')."' 
...
<tr>';
		...
		if(!empty($list_d_penjualan))
		{
				...
				foreach($list_result as $row)
				{
					...
					echo'<tr id="'.$row->id_produk.''.$row->id_h_diskon.'" >';
						...
						{
							echo'<td style="text-align:right;"><input type="text" name="harga-'.$row->id_produk.'-'.$row->id_h_diskon.'" id="harga-'.$row->id_produk.'-'.$row->id_h_diskon.'" style="text-align:right;" maxlength="15" size="7" onkeypress="return isNumberKey(event)" onchange="edit_d_penjualan(this.id)" onfocusout="getRupiah(this.id)" class="required select2" value="'.number_format($row->harga,0,'.',',').'" disabled="true"/></td>';
						...
						echo'<td><input type="text" name="aturan_minum-'.$row->id_produk.'-'.$row->id_h_diskon.'" id="aturan_minum-'.$row->id_produk.'-'.$row->id_h_diskon.'" style="text-align:right;" maxlength="35" size="18" class="required select2" onchange="edit_d_penjualan(this.id)" value="'.$row->aturan_minum.'" /> </td>';
						...
						echo'<input type="hidden" id="satuan_jual_old-'.$row->id_produk.'-'.$row->id_h_diskon.'" name="satuan_jual_old-'.$row->id_produk.'-'.$row->id_h_diskon.'" value="'.$row->satuan_jual.'" />';
	...
	{
		$this->M_gl_h_penjualan->ubah_h_penjualan_media_transaksi($_POST['id_h_penjualan'],$this->session->userdata('ses_kode_kantor'),$_POST['media_transaksi'],$_POST['diagnosa']);
