...
class C_admin_jabatan extends CI_Controller {
	...
	public function hapus()
	{
		...
		$data_jabatan = $this->M_jabatan->get_jabatan('id_jabatan',$id);
		if(!empty($data_jabatan))
		{
			...
			$this->M_log->simpan(
									...
									"Melakukan penghapusan jabatan ".$data_jabatan->nama_jabatan." dengan id : ".$data_jabatan->id_jabatan." ",
	...
	public function list_hak_akses()
	{
		...
		if((!empty($_GET['cari'])) && ($_GET['cari']!= "")  )
		{
			$cari = "WHERE (
								...
								A.nama_fasilitas LIKE '%".str_replace("'","",$_GET['cari'])."%'
								OR A.nama_group1 LIKE '%".str_replace("'","",$_GET['cari'])."%'
							)
					";	
		}
		else
